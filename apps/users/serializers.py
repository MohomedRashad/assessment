...
def create_user(validated_data):
    ...
    if validated_data.get('role') == 'PHARMACY':
        ...
        if not all([validated_data.get('pharmacy_name'), validated_data.get('postal_code')]):
            instance.delete() #deleting the already created user instance before raising an error
            raise ValidationError('Please provide pharmacy name, and postal code for pharmacy users')
        else:
            #creating a pharmacy object for the created user.
