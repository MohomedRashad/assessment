...
def create_order(type, **kwargs):
    ...
    if type == 'FORM_ASSESSMENT':
        if 'form_assessment' in kwargs:
            ...
            order = Order(type = type, form_assessment = kwargs.get('form_assessment'), total_amount = kwargs.get('total_amount'))
            order.save()
    elif type == 'VIDEO_ASSESSMENT':
        if 'appointment' in kwargs:
            #Creating the VIDEO_ASSESSMENT order
            order = Order(type = type, appointment = kwargs.get('appointment'), total_amount = kwargs.get('total_amount'))
            order.save()
    else:
